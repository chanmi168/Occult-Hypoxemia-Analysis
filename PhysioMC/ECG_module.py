...
def unpack_ECG(i_PT_peaks, ecg_dict, offset_SR, verbose=False):    
    Fs = ecg_dict['Fs']
...
def QRS_detector_dict_diagnostics(QRS_detector_dict_patch, mask_sqi_patch, mask_hr_patch, Fs, beat_name='i_R_peaks',fig_name=None, outputdir=None, show_plot=False):
    ...
    fig, (ax1, ax2) = plt.subplots(2,1, figsize=(t_dur*0.5,5),dpi=80)

    ax1.plot(t_arr, ECG_arr,alpha=0.5)

    ax1.scatter(i_beat_peaks_patch/Fs, ECG_arr[i_beat_peaks_patch],s=10, c='g', alpha=0.5, marker='o')
    ax1.plot(i_beat_peaks_patch/Fs, ECG_arr[i_beat_peaks_patch], c='g', alpha=0.5, marker='o')
...
def task_HR_detector(ECG_raw_patch, Fs, fig_name=None, outputdir=None, show_plot=True):
...
#     show_plot = True
    ...
    if debug:
        ...
        QRS_detector_dict_diagnostics(QRS_detector_dict_patch, mask_fake, mask_fake, Fs, beat_name=beat_choice,
    ...
    sqi_xcorr = get_sqi(ecg_beats, get_innerproduct, template)
    med_deviation = np.abs(get_padded_medfilt(sqi_xcorr, k=5, offset=5) - sqi_xcorr)
...
#     print(ts_hr_patch.shape[0])
    ...
    if outputdir is not None or show_plot is not None:
        QRS_detector_dict_diagnostics(QRS_detector_dict_patch, mask_sqi_patch, mask_hr_patch, Fs, beat_name=beat_choice,
